 ...
 * @editer ZhuoyuanZheng@ronghai.com
 */
...
sd.initPara = function(para){
  ...
  if(typeof sd.para.server_url === 'string'){
    sd.para.debug_mode_url = sd.para.debug_mode_url || sd.para.server_url+((sd.para.server_url.indexOf("?")>0)?"&":"?")+"dcsverbose=true"; //郑卓源修改
  } 
  // 是否需要非cache，等于每次请求文件
 ...
 * 郑卓源增加代码
 */
  _.exre=function(){return window.RegExp?new RegExp("dcs(uri)|(ref)|(aut)|(met)|(sta)|(sip)|(pro)|(byt)|(dat)|(p3p)|(cfg)|(redirect)|(cip)","i"):""}();
  _.re=function(){return window.RegExp?{"%25":/\%/g,"%09":/\t/g,"%20":/ /g,"%23":/\#/g,"%26":/\&/g,"%2B":/\+/g,"%3F":/\?/g,"%5C":/\\/g,"%22":/\"/g,"%7F":/\x7F/g,"%A0":/\xA0/g}:""}();
  
  _.dcsEscape = function(S, REL) {
    if (REL != "") {
        S = S.toString();
        for (var R in REL) {
  ...
  _.dcsA = function(N, V) {
      if ((_.exre != "") && !_.exre.test(N)) {
          ...
          V = this.dcsEncode(_.trim(V));
      } else V = this.dcsEncode(this.dcsEscape(V, this.re));
      return "&" + N + "=" + V;
  };
  _.dcsEncode = function(S) {
      return (typeof(encodeURIComponent) == "function") ? encodeURIComponent(S) : escape(S);
  };
  _.dcsGetUrlInfo=function(url) {
    var o = document.createElement('href'),
        result;
    o.href = url;
    url = o.href;
...
sd.sendState.getSendCall = function(data, callback) {
  ...
  /**郑卓源增加代码 */
  var u = _.dcsGetUrlInfo(location.href);
  var n="/"
  if(data["properties"]["$element_content"]){n+=data["properties"]["$element_content"]}
  else if(data["properties"]["$element_name"]){n+=data["properties"]["$element_name"]}
  else if(data["properties"]["$element_id"]){n+=data["properties"]["$element_id"]}
  data["dcsuri"]=u.pathname+'/'+data["event"]+n;
  data["dcsqry"] = u.search + u.hashSearch;
  data["dcssip"] = u.hostname;
  data["hash"] = u.hash;
  data["hashpathname"] = u.hashPathname;
  data["dcsref"]=data["properties"]?data["properties"]["$referrer"]:'';
  /**郑卓源增加代码 */
...
sd.sendState.getUrlPara = function(url,data){
  var WS = eval('(' + data + ')');
  var P="http"+(window.location.protocol.indexOf('https:')==0?'s':'')+"://"+url+(url.indexOf("?")>0?"":"?");
  for (var N in WS){
    if (WS[N]&&N.indexOf('_')!=0&&!_.isObject(WS[N])) {
      P+=_.dcsA(N,WS[N]);
    }
  }
  for (N in WS["properties"]){
    if (WS["properties"][N]) {
      P+=_.dcsA("properties."+N,WS["properties"][N]);
    }
  }
  for (N in WS["lib"]){
    if (WS["lib"][N]) {
      P+=_.dcsA("lib."+N,WS["lib"][N]);
    }
  }
...
saEvent.send = function(p, callback) {
  ...
  saEvent.debugPath = function(data, callback) {
    ...
    var WS = eval('(' + data + ')');
    var P="http"+(window.location.protocol.indexOf('https:')==0?'s':'')+"://"+sd.para.debug_mode_url;
    for (var N in WS){
      if (WS[N]&&N.indexOf('_')!=0&&!_.isObject(WS[N])) {
        P+=_.dcsA(N,WS[N]);
      }
